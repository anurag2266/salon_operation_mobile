...
const AddSalonDetail = ({navigation}) => {
  ...
  const handleSubmitSalonDetails = async () => {
    if (salonDetail.name.length < 2) {
    ...
    } else if (!salonDetail.companyType || salonDetail.companyName == '') {
      showMessage({
        message: 'Please Choose Company Type',
        type: 'danger',
      });
    } else if (salonDetail.companyName == '' || !salonDetail.companyName) {
      showMessage({
        message: 'Salon Company Name is required',
        type: 'danger',
      });
      ...
      const {data, message, status} = await addSalonDetailsAPI(salonDetail);
      if (status) {

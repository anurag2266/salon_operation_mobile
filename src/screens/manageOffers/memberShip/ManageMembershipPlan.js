...
const ManageMembershipPlan = ({navigation, route}) => {
        ...
        {membershipPlans.length == 0 ? (
          ...
          <View style={{marginHorizontal: vw(23), marginTop: vh(40)}}>
            {membershipPlans.map(item => (
              <MembershipCard
                onPressViewPlan={() => navigation.navigate('CardHistory')}
                onPressLog={() => navigation.navigate('ViewLogHistory')}
                onPressSaleHistory={() => navigation.navigate('SaleHistory')}
                onPressAvailHistory={() =>
            ...
            <TouchableOpacity
              onPress={() => {
                navigation.navigate('AddMembershipPlan');
              }}>
              <Text style={styles.createNewText}>{'+ Create New Membership'}</Text>
            </TouchableOpacity>
          </View>
        )}
      </Container>

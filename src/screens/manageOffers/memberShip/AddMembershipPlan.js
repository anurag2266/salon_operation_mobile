...
import ApplicableOnService from './ApplicableOnService';
import AddServices from '../../salon/address/salonServices/AddServices';

const AddMembershipPlan = ({navigation, route}) => {
  ...
  const [clubingWithItems, setclubingWithItems] = useState(false);
  ...
  const handleMembershipPlan = async () => {
    let vald = {
      expiration: myValidity.expiration,
      enterNo: myValidity.enterNo,
      saleStartFrom: myValidity.saleStartFrom,
      saleEndOn: myValidity.saleEndOn,
      redemptionSetting: [
        {
          for: redemptionType,
    ...
    bodyData.append('validity', JSON.stringify(vald));
    bodyData.append('planapplicablefor', membershipPlan.planapplicablefor);
    bodyData.append('name', membershipPlan.name);
    bodyData.append('membershipType', membershipPlan.membershipType);
    bodyData.append('membershipTag', membershipPlan.membershipTag);
    bodyData.append('membercount', membershipPlan.membercount);
    bodyData.append('sacCode', membershipPlan.sacCode);
    bodyData.append('applicableOn', membershipPlan.applicableOn);
    bodyData.append('discountType', membershipPlan.discountType);
    bodyData.append('allowFor', membershipPlan.allowFor);
    bodyData.append('planPrice', membershipPlan.planPrice);
    bodyData.append('valueWithTax', membershipPlan.valueWithTax);
    bodyData.append('shortDescription', membershipPlan.shortDescription);
    bodyData.append('termandconditions', membershipPlan.termandconditions);
    bodyData.append('tandcfile', membershipPlan.imageUrl);
    bodyData.append('isAutoApply', appliedOn);
    bodyData.append('isClubingwithoffers', clubing);
    bodyData.append('isClubingwithpurchaseItems', clubingWithItems);
    if (file) {
    ...
    const {data, status, message} = await addMembershipData(bodyData);
    if (status) {
    ...
    <View style={{flex: 1, backgroundColor: theme.color.white}}>
      <Container
        ...
        <Toggle
          ...
          detail={'Manage Membership Validity?'}
        />
        {file ? (
        ...
        <Dropdown
          ...
          onChange={item => {
            setmembershipPlan({...membershipPlan, membershipType: item.value});
          }}
        />
        <TextInput
...
<TextInput
        ...
        <Text style={styles.textButton}>View Applicable Services</Text>
        <TextInput
        ...
        <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
          <TextInput
            value={myValidity.saleStartFrom}
            style={[styles.input, {width: '40%'}]}
            inputStyle={styles.inputStyle}
            labelStyle={styles.labelStyle}
            placeholderStyle={styles.placeholderStyle}
            ...
            focusColor={theme.color.borderGrey}
            onChangeText={text => {
              setmyValidity({...myValidity, saleStartFrom: text});
            }}
          />
          <TextInput
            value={myValidity.saleEndOn}
            style={[styles.input, {width: '40%'}]}
            inputStyle={styles.inputStyle}
            labelStyle={styles.labelStyle}
            placeholderStyle={styles.placeholderStyle}
            ...
            focusColor={theme.color.borderGrey}
            onChangeText={text => {
              setmyValidity({...myValidity, saleEndOn: text});
            }}
          />
        </View>
      ...
      />
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            setappliedOn(false);
            setClubing(false);
            setclubingWithItems(!clubingWithItems);
          }}>
          {clubingWithItems ? (
            <Image style={styles.checkimg} source={LocalImages.checked} />
          ) : (
          ...
          <Text style={styles.labelText}>
            Allow clubing with other purchased items
          </Text>
        </TouchableOpacity>
        <RBSheet
          ref={refRBSheet}

...
import { getAllComboOffers } from '../../../api/comboOffers/addComboOffers';

const ComboOffers = ({navigation, route}) => {

  const [couponView, setcouponView] = useState(['', '']);
  useFocusEffect(
    React.useCallback(()=>{
      getAllComboOffers().then((res)=>{
          ...
          <View style={{marginHorizontal: vw(23), marginTop: vh(40)}}>
            {couponView.map(item => (
              <ComboOfferCard
                data={item}
                onPressViewDetail={() => {
                  navigation.navigate('CreateComboOffers'

...
const ManageSalonOperation = ({navigation, route}) => {
  ...
  const handleGetUserInfo = async () => {
    ...
    if (status) {
      ...
      //   setsellProduct(data.salons.isSalonSellProducts);
      //   stylistAttendance(data.salons.manageStylistAttendance);
      //   setautoBooking(data.salons.autoAccept);
      //   setHomeServiceRange(data.salons.HomeServiceRange);
      //   setHomeService(data.salons.isHomeService);
      //   setextraServicecharge(data.salons.isExtraFeesForHomeService);
  ...
  const handleUpdateSalonOperations = async () => {
    const {data, message, status} = await updateBusinesSetupAPI({
      ...
      isExtraFeesForHomeService: extraServicecharge,
    });
...
export default ManageSalonOperation;
